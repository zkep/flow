# Flow - 工作流编排库

🌍 **语言切换**: [English Documentation](README.md)

Flow 是一个强大的 Go 语言工作流构建和执行库，提供两种执行模式：线性执行链（Chain）和图形化执行器（Graph）。

## 功能特性

### Chain 模式

- **线性执行**: 按顺序执行任务，步骤依次运行
- **自动参数传递**: 上一步输出自动作为下一步输入
- **错误处理**: 全面的错误传播和处理机制
- **简单易用**: 适合简单的顺序处理场景

### Graph 模式

- **图形工作流**: 使用节点（Node）和边（Edge）构建复杂工作流
- **多种边类型**: 支持普通边、循环边和分支边
- **条件执行**: 为边添加条件以控制工作流路径
- **并行执行**: 并发执行独立节点以提高性能
- **自动参数处理**: 任务间智能参数传递和类型转换
- **错误处理**: 全面的错误传播和处理机制
- **检查点支持**: 保存和恢复工作流状态以实现容错
- **暂停/恢复**: 暂停工作流执行并在稍后恢复
- **可视化支持**: 生成 Mermaid 和 Graphviz 图表用于工作流可视化

## 核心概念

### 节点 (Node)

**节点** 代表工作流中的单个任务或操作。每个节点包含：

- **name**: 节点的唯一标识符
- **fn**: 要执行的函数
- **status**: 当前执行状态（pending、running、completed、failed）
- **inputs/outputs**: 与其他节点的数据流转连接
- **result**: 执行后的返回值

```go
// 添加一个带函数的节点
g.AddNode("calculate", func(x int) int {
    return x * 2
})
```

### 边 (Edge)

**边** 定义节点之间的连接和数据流。边控制：

- **from/to**: 源节点和目标节点名称
- **edgeType**: 普通、循环或分支执行流程
- **condition**: 可选的条件函数，用于控制执行路径
- **weight**: 执行优先级

```go
// 基本边 - 数据从 start 流向 process
g.AddEdge("start", "process")

// 条件边 - 仅在条件为真时执行
g.AddEdge("check", "action", flow.WithCondition(func(result []any) bool {
    return result[0].(int) > 10
}))

// 分支边 - 用于条件分支
g.AddEdge("decision", "branchA", flow.WithEdgeType(flow.EdgeTypeBranch))
```

### 协同工作方式

```
[节点 A] ──边──> [节点 B] ──边──> [节点 C]
   │                                    ↑
   └─────────────边─────────────────────┘
```

1. **节点** 执行实际的工作（函数）
2. **边** 定义执行顺序和数据传递
3. 当节点 A 完成后，其输出通过边传递给节点 B
4. 边的条件决定节点 B 是否应该执行

## 基准测试结果

在 Apple M1 Pro (darwin/arm64) 上的基准测试结果：

| 基准测试 | 迭代次数 | 时间 (ns/op) | 内存 (B/op) | 分配次数 (allocs/op) |
|-------------|------------|----------------|----------------|-------------------------|
| BenchmarkC32-8 | 69,494 | 16,679 | 3,987 | 37 |
| BenchmarkS32-8 | 418,197 | 2,687 | 2,976 | 34 |
| BenchmarkC6-8 | 193,282 | 5,968 | 1,179 | 16 |
| BenchmarkC8x8-8 | 15,262 | 78,383 | 9,027 | 125 |

**基准测试说明：**

- **C32**: 32 个独立节点（完全并行执行）
- **S32**: 32 个节点顺序链
- **C6**: 6 个节点的菱形依赖结构
- **C8x8**: 8 层 x 8 节点深度网络（每层完全连接到下一层）

## 安装

```bash
go get github.com/zkep/flow
```

## 快速开始

### 基础 Chain 示例

```go
package main

import (
    "fmt"
    "github.com/zkep/flow"
)

func main() {
    chain := flow.NewChain()

    chain.Add("step1", func() int {
        return 10
    })

    chain.Add("step2", func(x int) int {
        return x * 2
    })

    chain.Add("step3", func(y int) int {
        return y + 5
    })

    err := chain.Run()
    if err != nil {
        fmt.Printf("错误: %v\n", err)
        return
    }

    result, err := chain.Value("step3")
    if err != nil {
        fmt.Printf("错误: %v\n", err)
        return
    }

    fmt.Printf("最终结果: %v\n", result) // 输出: 25
}
```

### 基础 Graph 示例

```go
package main

import (
    "fmt"
    "github.com/zkep/flow"
)

func main() {
    g := flow.NewGraph()

    g.AddNode("start", func() int {
        return 10
    })

    g.AddNode("process1", func(x int) int {
        return x * 2
    })

    g.AddNode("process2", func(x int) int {
        return x + 5
    })

    g.AddNode("end", func(x int) {
        fmt.Printf("最终结果: %d\n", x)
    })

    g.AddEdge("start", "process1")
    g.AddEdge("process1", "process2")
    g.AddEdge("process2", "end")

    err := g.Run()
    if err != nil {
        fmt.Printf("错误: %v\n", err)
    }
}
```

## 文档

完整文档请查看 [docs/zh-CN/doc.md](docs/zh-CN/doc.md)。

## 示例

查看 [_examples](_examples) 目录获取更多示例：

- [basic-chain](_examples/basic-chain/main.go) - 基础 Chain 使用
- [basic-graph](_examples/basic-graph/main.go) - 基础 Graph 使用
- [advanced-chain](_examples/advanced-chain/main.go) - 高级 Chain 特性
- [advanced-graph](_examples/advanced-graph/main.go) - 高级 Graph 特性
- [approval-flow](_examples/approval-flow/main.go) - 实际审批流程示例

## 许可证

MIT License
