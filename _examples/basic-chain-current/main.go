...
func main() {
	fmt.Println("=== Basic Chain Examples (Current Implementation) ===\n")
...
func example5TypeConversion() {
	fmt.Println("5. Type Conversion Pipeline")
	fmt.Println("   -------------------------")
...
func example6ErrorHandling() {
	fmt.Println("6. Error Handling in Chains")
	fmt.Println("   -------------------------")
...
func example7UsingSteps() {
	fmt.Println("7. Using Steps to Combine Results")
	fmt.Println("   -------------------------------")
	...
	newChain2 := chain.Use("step1", "step2")

	newChain2.Add("combined", func(a, b int) int {
		return a + b
	})
	...
	if err != nil {
		...
		result, err := newChain2.Value("combined")
		if err != nil {
			fmt.Printf("   Error getting result: %v\n", err)
			return
		}
